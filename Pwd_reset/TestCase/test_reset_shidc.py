...
@allure.title("IDC修改admin密码成功！修改后密码：eisoo.com123")
...
def test_resetsh_admin_once(shidc_login_admin, resetadmin, expected):
    ...
    cookie = shidc_login_admin.cookies
    userId = shidc_login_admin.cookies['userId']
    referer = res['shidc_reset_headers']['Referer']
    csrftoken = shidc_login_admin.cookies['csrftoken']
    header = {"referer": "%s" % (referer),
              "x-csrftoken": "%s" % (csrftoken)}

    resetadmin['userId'] = userId
    url = parse.urljoin(res['shidc_base_url'], res['reset_url']).format(userId)
...
def test_resetsh_admin_twice(resetadmin, expected):
    file = open('D:\workspace\Pwd_reset\TestData\config.yaml')
    res = yaml.load(file, Loader=yaml.FullLoader)
    ...
    referer = res['shidc_reset_headers']['Referer']
    csrftoken = shidc_login_response.cookies['csrftoken']
    reset_header = {"referer": "%s" % (referer),
              "x-csrftoken": "%s" % (csrftoken)}

    resetadmin['userId'] = userId
    reset_url = parse.urljoin(res['shidc_base_url'], res['reset_url']).format(userId)
...
def test_resetsh_sadmin_once(shidc_login_sadmin, resetsadmin, expected):
    file = open('D:\workspace\Pwd_reset\TestData\config.yaml')
    res = yaml.load(file, Loader=yaml.FullLoader)

    # 获取修改密码需要的参数：method、header、resetsadmin、cookie、url
    method = res['reset_method']
    cookie = shidc_login_sadmin.cookies
    userId = shidc_login_sadmin.cookies['userId']
    referer = res['shidc_reset_headers']['Referer']
    csrftoken = shidc_login_sadmin.cookies['csrftoken']
    header = {"referer": "%s" % (referer),
              "x-csrftoken": "%s" % (csrftoken)}
...
def test_resetsh_sadmin_twice(resetsadmin, expected):
    file = open('D:\workspace\Pwd_reset\TestData\config.yaml')
    res = yaml.load(file, Loader=yaml.FullLoader)
...
@allure.title("IDC修改操作员密码成功！修改后密码：eisoo.com123")
...
def test_resetsh_operator_once(shidc_login_operator, resetoperator, expected):
    file = open('D:\workspace\Pwd_reset\TestData\config.yaml')
    res = yaml.load(file, Loader=yaml.FullLoader)

    # 获取修改密码需要的参数：method、header、resetsadmin、cookie、url
    method = res['reset_method']
    cookie = shidc_login_operator.cookies
    userId = shidc_login_operator.cookies['userId']
    referer = res['shidc_reset_headers']['Referer']
    csrftoken = shidc_login_operator.cookies['csrftoken']
    header = {"referer": "%s" % (referer),
              "x-csrftoken": "%s" % (csrftoken)}
